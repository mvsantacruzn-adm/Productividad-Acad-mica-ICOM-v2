...
function construirTabla(titulo, headers, filas) {
    // ESPECIAL: Si es tabla de Proyectos, aplicar filtros y ocultar columnas
    if (titulo === 'Proyectos de Investigación') {
        return construirTablaProyectos(titulo, headers, filas);
    }
...
function construirTablaProyectos(titulo, headers, filas) {
    // PASO 1: Filtrar proyectos según criterios
    const filasFiltradasProyectos = filas.filter(fila => {
        // 1. Año >= 2020
        const ano = parseInt(fila['Año'] || 0);
        if (ano < 2020) return false;
        
        // 2. Fuente de financiamiento contiene "ANID"
        const fuente = (fila['Fuente de financiamiento'] || '').toUpperCase();
        if (!fuente.includes('ANID')) return false;
        
        // 3. Duración >= 2 años (si está disponible, si no, mantener)
        const duracion = fila['Duración'];
        if (duracion !== '' && duracion !== undefined && duracion !== null) {
            const duracionNum = parseInt(duracion);
            if (duracionNum < 2) return false;
        }
        
        // 4. USD >= 5000 (si está disponible, si no, mantener)
        const usd = fila['USD'];
        if (usd !== '' && usd !== undefined && usd !== null) {
            const usdNum = parseInt(usd);
            if (usdNum < 5000) return false;
        }
        
        return true;
    });
    
    // PASO 2: Si no hay proyectos después del filtro, no mostrar tabla
    if (filasFiltradasProyectos.length === 0) {
        return '';
    }
    
    // PASO 3: Ocultar columnas: CLP, TC, Observaciones
    const columnasOcultas = ['CLP', 'TC', 'Observaciones'];
    const headersVisibles = headers.filter(h => !columnasOcultas.includes(h));
    
    // PASO 4: Construir tabla con solo columnas visibles
    let html = `
        <h3 style="font-size: 11px; font-weight: bold; color: #333; margin-top: 18px; margin-bottom: 8px;">${titulo}</h3>
        <table class="ficha-table" style="width: 100%; border-collapse: collapse; margin-bottom: 18px; border: 1px solid #ddd; font-size: 10px;">
            <thead>
    ...
    filasFiltradasProyectos.forEach((fila, idx) => {
        html += `<tr>`;
...
function inicializarDocumentos() {
    ...
    for (const doc of documentos) {
        // Construir URLs exactas
        const urlPreview = './' + doc.filename;
        const urlDownload = './' + doc.filename;
        ...
        fila.innerHTML = `
            ...
            <td style="padding: 10px; text-align: center; border: 1px solid #ddd;">
                <button class="btn-secondary" onclick="previsualizarDocumento('${doc.filename}')" style="padding: 5px 10px; font-size: 11px; display: block; margin-bottom: 5px;">
                    👁️ Ver
                </button>
                <span style="font-size: 9px; color: #666; word-break: break-all; display: block; padding-top: 5px; border-top: 1px solid #eee; margin-top: 5px;">
                    ${urlPreview}
                </span>
            </td>
            <td style="padding: 10px; text-align: center; border: 1px solid #ddd;">
                <button class="btn-secondary" onclick="descargarDocumento('${doc.filename}')" style="padding: 5px 10px; font-size: 11px; display: block; margin-bottom: 5px;">
                    📥 Descargar
                </button>
                <span style="font-size: 9px; color: #666; word-break: break-all; display: block; padding-top: 5px; border-top: 1px solid #eee; margin-top: 5px;">
                    ${urlDownload}
