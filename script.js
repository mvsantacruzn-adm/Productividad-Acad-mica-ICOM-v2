...
function descargarFichasClaustroExcel() {
    ...
    try {
        ...
        for (const nombreProfesor of profesoresOrdenados) {
            ...
            // INFORMACIÓN PERSONAL (sin columna Sección)
            datosConsolidados.push({
                'Campo': 'Nombre',
                'Valor': base.nombre || ''
            });
            ...
            for (const tipoSeccion in secciones) {
                ...
                console.log(`  → ${tipoSeccion}: ${seccion.filas.length} registros`);
                
                // Encabezado de sección
                const encabezados = seccion.headers || [];
                const filaEncabezado = { 'Sección': tipoSeccion };
                for (const header of encabezados) {
                    filaEncabezado[header] = '';
                }
                datosConsolidados.push(filaEncabezado);
                
                // Datos de la sección
                for (let i = 0; i < seccion.filas.length; i++) {
                    const fila = seccion.filas[i];
                    const filaData = { 'Sección': '' };
                    
                    for (const header of encabezados) {
                        filaData[header] = String(fila[header] || '');
                    }
                    datosConsolidados.push(filaData);
                }
        ...
        console.log(`✓ Total de filas: ${datosConsolidados.length}`);
        ...
        // Generar XLSX con bordes
        const timestamp = obtenerTimestampDescarga();
        generarXLSXConBordes(datosConsolidados, `Fichas_CNA_Claustro_Completo_${timestamp}.xlsx`);
...
function generarXLSXConBordes(datos, nombreArchivo) {
    console.log('→ generarXLSXConBordes:', nombreArchivo);
    ...
    try {
        ...
        for (let R = range.s.r; R <= range.e.r; ++R) {
            for (let C = range.s.c; C <= range.e.c; ++C) {
                ...
                // Fondo gris para headers (primera fila)
                if (R === 0) {
        ...
        console.error('❌ Error en generarXLSXConBordes:', error);
        throw error;
    }
}
